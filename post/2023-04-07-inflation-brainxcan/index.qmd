---
title: "Polygenicity Driven Inflation in Brainxcan"
author: "Haky Im"
date: "2023-04-07"
categories: [analysis]
...
When both $Y$ and a mediator IDP are polygenic, the regression test $Y$ on IDP is inflated with $\text{Var}(Z_\text{bxcan}) = 1 + h_Y^2 \cdot N \cdot \frac{\text{tr}(R'R)}{\text{tr}^2(R)}$. I'll use simulations to understand this relationship as functions of $h_Y$, $h_\text{IDP}$, $N$, $M_Y$, and $M_\text{IDP}$. The IDP has been assumed to be known without error in this derivation, i.e. $h_\text{IDP}=1$.
:::

-   $h^2_Y$ is the heritability of $Y$
-   $h^2_\text{IDP}$ is a measure of how close the predicted IDP is to the genetic component of the IDP
...
::: callout-tip
## Inflation estimate with uncertainty in IDP prediction

$\text{Var}(Z_\text{bxcan}) = 1 + h_Y^2 \cdot N \cdot h^2_\text{IDP} \cdot \frac{\text{tr}(R'R)}{\text{tr}^2(R)}$
:::

## how to correct the inflation

We will correct the inflation by dividing the association Z-score with the $\sqrt{\text{Var}(Z_\text{bxcan})}$. In practice, we will estimate the factor $$h^2_\text{IDP} \cdot \frac{\text{tr}(R'R)}{\text{tr}^2(R)}$$ by simulating traits Y with different heritability and different sample sizes, calculating the Z-scores of the association with IDPs, and regressing the mean Z-score squared across multiple simulations of Y on $h^2_Y$.


## - [ ] TODO: test whether more sparse architectures can be accounted for in the variance formula by calculating

$$\frac{\text{tr}(R'R)}{\text{tr}^2(R)}$$ using only SNPs that are causal for both? Or some function of $M_\text{IDP}$, $M_Y$, and $M$ of the intersection.
...
simulate_study = function(nsam,msnp,nsim,midp,h2IDP,h2Y,prop_causal)
{
...
if(rerun)
{
  ...
  for(ii in 1:num_nsam)
  {
    for(jj in 1:num_msnp)
    {
      ...
      if(noskip) 
      {
        res = simulate_study(nsam,msnp,nsim,midp,h2IDP,h2Y,prop_causal)
...
## - [ ] TODO: show that this inflation does not affect sparse traits like the cis components of gene expression

```{r inflation with cis predicted gene expression}
