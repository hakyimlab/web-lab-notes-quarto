---
title: "TESTING Polygenicity Driven Inflation in Brainxcan II"
author: "Haky Im"
date: "2023-08-23"
categories: [analysis]
...
simulate_study = function(nsam,msnp,nsim,midp,precision_gamma,h2Y,prop_causal)
{
  ...
  if(b_cond)
  {
    betamat = matrix(betamat[,1],msnp, nsim)
  }
...
if(rerun)
{
  ## define name of files to save arrays
  varfilehead ="{DATA}/varZarray-cor2t"
  trrfilehead ="{DATA}/trrtrZarray-cor2t"
  ## prep parallel runs
  tic = Sys.time()
  print(tic)
  ...
  saveRDS(varZarray,glue("{varfilehead}-h2-{h2Y}.RDS"))
  saveRDS(trrtrarray,glue("{trrfilehead}-h2-{h2Y}.RDS"))
...
varZarray = readRDS(glue("{varfilehead}-h2-{h2Y}.RDS"))
trrtrarray = readRDS(glue("{trrfilehead}-h2-{h2Y}.RDS"))

for(hh in 2:num_h2Y)
{
  h2Y=h2Yvec[hh]
  varZarray_hh = readRDS(glue("{varfilehead}-h2-{h2Y}.RDS"))
  trrtrarray_hh = readRDS(glue("{trrfilehead}-h2-{h2Y}.RDS"))
